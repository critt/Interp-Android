...
//import com.critt.trandroidlator.data.BuildConfig
...
class TranslationSource(
    ...
    fun connectObject(languageSubject: String, languageObject: String): Flow<com.critt.trandroidlator.domain.SpeechData> {
        /**
    ...
    fun connectSubject(languageSubject: String, languageObject: String): Flow<com.critt.trandroidlator.domain.SpeechData> {
        /**
         * https://socketio.github.io/socket.io-client-java/initialization.html#auth
         * */
        val options: IO.Options = IO.Options.builder().setAuth(mapOf("token" to sessionManager.getAuthToken())).build()
    ...
    private fun initSocket(socket: Socket?, config: Map<String, Any>): Flow<com.critt.trandroidlator.domain.SpeechData> = callbackFlow {
        socket?.connect()
        socket?.emit("startGoogleCloudStream", Gson().toJson(config))

        socket?.on("speechData") { args ->
            Gson().fromJson(args[0].toString(), com.critt.trandroidlator.domain.SpeechData::class.java).let {
